...
lastmod: 2019-01-04T18:49:00+08:00
draft: false
tags: ["docker", "network"]
categories: ["Docker"]
---
...
Docker容器将软件包装在一个完整的文件系统中，该文件系统包含了:代码、运行时、系统工具、系统库。
容器保证了软件运行环境的一致。
默认情况下Docker容器将应用程序与底层基础架构彼此隔离，还为应用程序提供了额外的保护层。

应用程序和主机或外部网络如何相互通信？
在保持应用程序可移植性，服务发现、负载平衡、安全性、性能以及可伸缩性的同时，如何设计容器网络？
本文解决了这些网络设计以及可用的工具和常见的部署模式的难题。
同时也提供了如何在应用程序和物理网络的束缚内设计Docker网络。

### 前提
在阅读本文之前，建议先学习下`Docker`和`Docker Swarm`：

* [Docker](https://docs.docker.com/engine/understanding-docker/)
* [Docker Swarm](https://docs.docker.com/engine/swarm/)与[Swarm模式](https://docs.docker.com/engine/swarm/key-concepts/#/services-and-tasks)


## 容器网络与微服务的挑战

微服务提升了应用程序的规模，使其更为重视应用程序的连接和隔离方法。 
Docker网络的理念受应用程序的驱动。旨在为网络提供多样性和灵活性，并适应开发人员的需要。

与任何设计一样，网络设计也是一种平衡做法。 
`Docker EE`和`Docker`生态系统为网络工程师提供了多种工具用以实现其应用程序和环境的最佳平衡。
每种选择有不同的优势与权衡。
余下的部分详细介绍了这些选择, 以此网络工程师可以了解到哪些选择最适合他们的网络环境。

Docker开发了一种新的应用程序交付方式，随之而来，容器也改变了某些网络的接入方式。

以下是容器化应用程序的常见设计模式：

* `Portability`(可移植性)

    如何利用网络特点保证跨多种网络环境的最大可移植性？

* `Service Discovery`(服务发现)
    
    当服务在扩充或缩减时如何知道该服务跑在哪个位置？
...
* `Security`(安全)
    ...
    如何细分以防止错误的容器相互访问？
    如何保证应用程序容器和集流量控制的容器是安全的？

* `Performance`(性能)

    在最小化延迟和最大化带宽的情况下如何提供高级的网络服务？

* `Scalability`(可扩展性)

    跨多主机扩展应用程序时如何确保这些特性不会被牺牲？

## 容器网络模型

Docker网络架构是构建在一组称为`Container Networking Model`(CNM)的接口上。 
`CNM`的理念是为各种基础设施提供应用程序的可移植性。 
该模型在实现应用程序可移植性以及利用基础设施结构的特殊功能方面找到平衡。
![](/images/attachment/590399-6ab22fb34e53a254.png)

### CNM组成

CNM中有几个高级组件。它们与操作系统和基础架构无关，因此无论基础架构如何，应用程序都能有统一的体验。

* `Sandbox` - Sandbox包含容器网络的配置。
配置包括容器接口的管理、路由表和DNS设置。 
Sandbox的实现可以是`Linux Network Namespace`(Linux网络命名空间)、`FreeBSD Jail`或其他类似的概念。
Sandbox可能包含多个网络的`Endpoint`。

* `Endpoint` - 端点将沙箱连接到网络。 Endpoint构造存在，因此可以从应用程序中抽象出与网络的实际连接。这有助于保持可移植性，以便服务可以使用不同类型的网络驱动程序，而无需关心它如何连接到该网络。

* `Network` -  CNM未根据OSI模型指定网络。网络的实现可以是Linux网桥，VLAN等。网络是端点的集合，它们之间具有连接。未连接到网络的端点在网络上没有连接。
...
在独立的Docker主机上，如果未指定其他网络，则bridge是容器连接的默认网络。在以下示例中，创建的容器没有网络参数。 Docker Engine默认将其连接到网桥。在容器内部，注意由桥驱动程序创建的eth0，并由Docker本机IPAM驱动程序给出一个地址。
```
...
brctl现在显示主机上的第二个Linux桥接器。 Linux网桥的名称br-4bcc22f5e5b9与my_bridge网络的网络ID匹配。 my_bridge还有两个连接到容器c2和c3的veth接口。
